...
describe('crud controllers', () => {
  describe('getOne', async () => {
    test('finds by authenticated user and id', async () => {
      ...
      const tag = await Tag.create({ name: 'tag', createdBy: user })
      ...
      const res = {
        ...
        json(result) {
          expect(result.data._id.toString()).toBe(tag._id.toString())
        }
      }

      await getOne(Tag)(req, res)
    })
  ...
  describe('getMany', () => {
    test('finds array of docs by authenticated user', async () => {
      ...
      const user = mongoose.Types.ObjectId()
      await Tag.create([
        { name: 'tag', createdBy: user },
        { name: 'other', createdBy: user },
        { name: 'tag', createdBy: mongoose.Types.ObjectId() }
      ])
  ...
  describe('updateOne', () => {
    test('finds doc by authenticated user and id to update', async () => {
      ...
      const user = mongoose.Types.ObjectId()
      const tag = await Tag.create({ name: 'name', createdBy: user })
      ...
      const res = {
        ...
        json(results) {
          expect(`${results.data._id}`).toBe(`${tag._id}`)
          expect(results.data.name).toBe(update.name)
        }
      }

      await updateOne(Tag)(req, res)
    })
